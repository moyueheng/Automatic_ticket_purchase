...
def check_login_status(login_cookies):
    ...
    response = requests.get(
        "https://passport.damai.cn/accountinfo/myinfo",
        headers=headers,
...
def get_api_param():
    ...
    js_code_define = requests.get(
        "https://g.alicdn.com/damai/??/vue-pc/0.0.70/vendor.js,vue-pc/0.0.70/perform/perform.js"
    ).text
    # 获取商品SKU的API参数
    commodity_param = re.search(
        'getSkuData:function.*?\|\|""}}', js_code_define
    ).group()
    commodity_param = re.search('data:{.*?\|\|""}}', commodity_param).group()
    commodity_param = (
        commodity_param.replace("data:{", "")
...
def get_select_seat_params(item_id, data_id=None):
    ...
    response = requests.get(
        "https://detail.damai.cn/subpage", headers=headers, params=params
    )
...
def get_seat_dynamic_info(cookies, project_id, item_id, perform_id):
    ...
    response = requests.get(
        "https://mtop.damai.cn/h5/mtop.damai.wireless.seat.dynamicinfo/1.0/",
        params=params,
        cookies=cookies,
        headers=headers,
    )
...
def get_select_seat_api(cookies, perform_id, city_id):
    ...
    response = requests.get(
        "https://mtop.damai.cn/h5/mtop.damai.wireless.project.getb2b2careainfo/1.3/",
        headers=headers,
        params=params,
        cookies=cookies,
    )
...
def get_valuable_seat_id(cookies, project_id, perform_id, city_id, stand_id):
    ...
    response = requests.get(
        "https://mtop.damai.cn/h5/mtop.damai.wireless.seat.queryseatstatus/1.0/",
        params=params,
        cookies=cookies,
        headers=headers,
    )
