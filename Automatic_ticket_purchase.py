...
class DaMaiTicket:
    ...
    def step1_get_order_info(self, item_id, commodity_param, ticket_price=None):
        ...
        response = self.session.get(
            "https://detail.damai.cn/subpage", headers=headers, params=commodity_param
        )
        ticket_info = json.loads(
            response.text.replace("null(", "").replace("__jp0(", "")[:-1]
        )
    ...
    def step2_click_buy_now(self, ex_params, sku_info):
        ...
        response = self.session.get(
            "https://buy.damai.cn/orderConfirm",
            headers=headers,
            params=params,
        ...
        try:
            submit_order_info = json.loads(
                result.group().replace("window.__INIT_DATA__ = ", "")[:-1]
            )
            submit_order_info.update(
                {"output": json.loads(submit_order_info.get("output"))}
            )
        except Exception:
            print(
                "-" * 10, "获取购买必备参数异常，请重新解析response返回的参数", "-" * 10
            )
            print(result.group())
            return False
        return submit_order_info

    def step2_click_confirm_select_seats(
        self, project_id, perform_id, seat_info, sku_info
    ):
        """选座购买，点击确认选座"""
        headers = {
            "authority": "buy.damai.cn",
    ...
    def step3_submit_order(self, submit_order_info, viewer, seat_info=None):
        ...
        params = (
            (
                "feature",
                '{"returnUrl":"https://orders.damai.cn/orderDetail","serviceVersion":"1.8.5"}',
            ),
            ("submitref", "undefined"),
        )
        ...
        if dm_viewer_pc_id_search:
            ...
            user_list = submit_order_info["data"][dm_viewer_pc_id]["fields"][
                "dmViewerList"
            ]
        ...
        if buy_status.get("success") is True and buy_status.get("module").get(
            "alipayOrderId"
        ):
            print(
                "-" * 10,
                "抢票成功, 请前往 大麦网->我的大麦->交易中心->订单管理 确认订单",
                "-" * 10,
            )
            print("alipayOrderId: ", buy_status.get("module").get("alipayOrderId"))
            print(
                "支付宝支付链接: ", buy_status.get("module").get("alipayWapCashierUrl")
            )
    ...
    def run(self):
        ...
        if os.path.exists("cookies.pkl"):
            ...
            self.login_cookies = tools.account_login(
                "account", self.login_id, self.login_password
            )
        else:
        ...
        if not login_status:
            print(
                "-" * 10,
                "登录失败, 请检查登录账号信息。若使用保存的cookies，则删除cookies文件重新尝试",
                "-" * 10,
            )
            return
        ...
        while True:
            ...
            ticket_sku_status = ticket_info["skuPagePcBuyBtn"]["skuBtnList"][
                sku_id_sequence
            ]["btnText"]
            if ticket_sku_status == "即将开抢":
                ...
                buy_serial_number = "{}_{}_{}".format(
                    self.item_id, self.buy_nums, sku_id
                )
                submit_order_info = self.step2_click_buy_now(
                    ex_params, buy_serial_number
                )
                break
            elif ticket_sku_status == "选座购买":
                # 获取选座购买必备的数据信息。
                city_id, project_id, item_id, perform_id = tools.get_select_seat_params(
                    self.item_id
                )
                stand_id, seat_price_list = tools.get_seat_dynamic_info(
                    self.login_cookies, project_id, item_id, perform_id
                )
                api_address = tools.get_select_seat_api(
                    self.login_cookies, perform_id, city_id
                )
                buy_serial_number = "{}_{}_{}".format(
                    self.item_id, self.buy_nums, sku_id
                )
                ...
                2. 过滤掉不可售的座位id。
                """
                valuable_seat = tools.format_valuable_seatid(
                    all_seats_info, valuable_info, price_id
                )
                # 挑选座位
